...
const createPostPlaceholders = (keyPrefix: string, count?: number): PostPlaceholder[] =>
...
const useInfiniteFeed = (
  ...
  const feedListing = useMemo(() => {
    const jsonPostList: PostFromJsonList =
      feedQuery?.data?.pages.map(page => page.posts).flat() || pageContext.feedPageMeta.posts;
    ...
    return list;
  }, [feedQuery.data, pageContext.feedPageMeta.posts, feedQuery.isFetchingNextPage]);
