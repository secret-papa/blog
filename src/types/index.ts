...
export function mdxNodeIntoPost(mdxNode: MdxNode): Post {
  ...
  if (!frontmatter)
    throw Error(
      `Post missing frontmatter. Post slug: ${mdxNode.fields?.slug || "not defined"}. Aborting.`,
    );

  if (!frontmatter.title)
    throw Error(
      `Post missing title. Post slug: ${mdxNode.fields?.slug || "not defined"}. Aborting.`,
    );
  ...
  if (!frontmatter.cover)
    throw Error(
      `Post missing cover image. Post slug: ${mdxNode.fields?.slug || "not defined"}. Aborting.`,
    );

  if (!frontmatter.coverAlt)
    throw Error(
      `Post missing cover alt. Post slug: ${mdxNode.fields?.slug || "not defined"}. Aborting.`,
    );
