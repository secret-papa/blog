...
export const getIndexListing = async (graphql: GraphqlType): Promise<PostList> => {
  const indexQueryResult = await graphql<MdxListingQuery>(indexListingQuery);

  return processQueryResult(indexQueryResult);
};

export const getTagListing = async (graphql: GraphqlType, tag: string): Promise<PostList> => {
...
export const getCategoryListing = async (
  ...
  const categoryQueryResult = await graphql<MdxListingQuery>(categoryListingQuery, { category });
