...
import { GatsbyPluginFeedData, GatsbyFeedRssMetadata, GatsbyFeedItem } from "../types";

export const getSerialize =
  (config: SiteConfig) =>
  (data: GatsbyPluginFeedData): Array<GatsbyFeedItem | undefined> | undefined => {
    const {
      query: { allMdx },
    } = data;
    ...
    const res = edges.map((edge): GatsbyFeedItem | undefined => {
      ...
      return {
        ...
        url,
        guid: url,
        custom_elements: [{ "content:encoded": node.html }, { author: config.user?.email }],
      };
    });
